  ...
  switch (action.type) {
    case "apply_snapshot": {
      ...
      const attributes = Object.fromEntries(
        (action.snapshot.attributes ?? []).map((item) => [item.id, item])
      );
      ...
      return normalizeUiSelections(
        updateServerState(state, (serverState) => ({
          ...
          attributes,
          attributeOrder: (action.snapshot.attributes ?? []).map((item) => item.id),
          augmentations,
          augmentationOrder: (action.snapshot.augmentations ?? []).map((item) => item.id),
          standaloneEffects,
          standaloneEffectOrder: (action.snapshot.standaloneEffects ?? []).map((item) => item.id),
