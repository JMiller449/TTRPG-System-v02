...
- Character sheets split the former compact Overview into expanded Stats and
  Statuses destinations. Stats preserves player allocation and GM grant/edit
  controls; Statuses preserves role-specific condition/effect visibility and
  GM condition removal.
- Character sheets default to a fixed Dense dashboard backed by authoritative
  runtime data. It supports staged player stat allocation and GM grants, real
  action pins and per-action rolls, action-point management, kill workflows,
...
- Dense Recent Kill Records use a single-column compact ledger: enemy and awarded XP remain prominent while timestamp, credit, participant count, and optional recorder share one secondary line. The full Kills destination retains its detailed cards.
...
- Frontend shell and navigation have been reorganized for table use: persistent session status, a shared role-filtered grouped sidebar, compact searchable action commands, and grouped GM workspaces. The sidebar uses one destination registry and the GM visual hierarchy for both roles; players see only Characters and Extension while backend authorization remains authoritative.
- Character sections are nested under Characters in the shared left sidebar. Opening Characters defaults to Dense; shared sections appear for both roles, while character Action History, Management, and Organize Sheets remain GM-only. The former horizontal sheet tab row was removed to increase usable workspace height and eliminate duplicate navigation.
- GM spawned-sheet context is integrated into the character workspace: the character name is the searchable active-sheet picker, and a GM-only Organize Sheets subsection renders folder organization inline. The former full-width context strip and organizer modal were removed to recover sheet height without changing frontend-local selection or catalog semantics.
- Authenticated workspaces use the full viewport without a duplicate title/status header. Backend, extension, Roll20, pending-request, History, and Exit controls live in a persistent shared-sidebar footer beneath the independently scrolling navigation; the History overlay opens upward from its lower-viewport trigger.
- The shared sidebar has a fixed role-filtered navigation search above its scroll region. Results include top-level workspaces and contextual character subsections, automatically highlight the top match, and support wrapping Up/Down selection, Enter activation, Escape clearing, and pointer selection.
- Player navigation presents exactly one active destination: opening the Extension
  tool collapses the Characters subtree; returning to Characters selects Dense.
- Character resistance destinations use a full-width responsive summary with separate
  core and damage-type groups instead of inheriting the generic horizontal form columns.
- Character inventory destinations keep the equipment list and item-proposal form in
...
- The frontend visual system now follows the dense R6 console reference layout: a full-height grouped role-aware rail with a persistent operational footer, non-blocking toast feedback, compact authoritative character workspace, vertical template section rail, and reusable catalog/editor authoring pages. This was a presentation-only pivot and did not add or remove application capabilities.
...
- The DM and player **Extension** workspace stages Violentmonkey installation, installs
  ...
  The sidebar status footer reports live userscript discovery as Extension
...
- The sidebar-footer Toast History panel renders through the document overlay layer while remaining
...
- XP workspace consistency (2026-08-29): Parties, Kill Registry, and XP Progress now share one lightweight inner-surface treatment for their primary navigation, entry, and management regions. The progress registry no longer falls back to unrelated ungrouped sections, while the full-width workspace remains free of redundant outer framing. Presentation-only; XP authority and protocol behavior are unchanged.
- Derived formula stats are integrated with their evaluated values on the character Stats destination.
...
- Template Builder scroll containment (2026-08-29): desktop builders keep the single workspace heading and footer actions anchored while the step rail and active section scroll independently inside the remaining console height. The redundant inner draft-title/status strip was removed; editing an existing template is identified by the workspace title, while control mode remains in Details and validation readiness remains in Review. Section navigation resets the active content to its top; mobile retains document-flow scrolling. Presentation-only; template authoring behavior is unchanged.
- Template Builder Attribute cleanup (2026-08-29): required and attached Attributes now render as compact summary cards, optional attachment opens the shared catalog dialog, and value/formula editing opens only after selecting a card. The section retains its contextual reusable-Attribute action and existing draft/default semantics.
