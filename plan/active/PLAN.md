...
- [x] Add typed Facts for sheets, items, and actions with backend-required defaults.
  ...
  - [x] Extend formula authoring metadata and validation for Fact formulas, including sheet-relative aliases, item-relative Fact aliases, action-relative Fact aliases, missing-reference errors, cycle detection across formula stats/Facts, and explicit backend evaluation failures.
    - Added backend-provided subject-aware Fact formula variables for sheet stats and compatible numeric Facts. Fact Builder plus sheet, item, action, and Template Builder Fact editors consume the same searchable metadata and insert relative aliases without frontend evaluation.
    - Fact definition and subject-value writes now reject aliases outside every declared subject scope, references to unattached Facts, and invalid authoritative evaluations before mutation.
    - Sheet formula dependency validation now spans formula stats and formula Facts. Atomic sheet create/update and individual stat/Fact edits reject missing Fact dependencies and stat-to-Fact or Fact-to-stat cycles without persisting broken state.
  ...
  - [x] Build a GM Fact Builder for optional definition CRUD and required Fact editing/reset. Required records must show their subject/profile scope and a clear Required badge while omitting rename/delete controls.
    - Optional definitions retain full create/edit/delete controls. Backend-owned required and preset definitions are visibly locked in the catalog, while their permitted subject bridge values/formulas are edited or reset from the relevant sheet/item/action Facts section.
    - [x] Optional definition CRUD, typed defaults, validation options, units, visibility, subject scope, metadata-backed relative formula aliases, and locked Required records are implemented. Required values remain subject-specific and are intentionally edited/reset on the owning sheet, item, or action rather than on the global definition record.
  - [x] Add a Facts section to Template Builder for attaching optional definitions and displaying locked required assignments; create/update must submit stable bridges in the complete sheet payload.
    - New template drafts seed backend-declared required sheet Facts with stable reserved relationship IDs. Optional Facts can be attached, edited, reset, or detached from the dedicated tab; required Facts cannot be detached.
    - Template create/update now sends the complete Fact bridge map atomically with stats and assignments. The backend validates bridge identity, definitions, subject compatibility, formula paths/dependencies, and evaluated results before replacing sheet state.
...
- [x] Seed canonical authoring data and prove the `dm_examples.md` examples through normal system primitives.
  - [x] Move the standard substat default formulas into backend-provided authoring metadata and make new Template Builder drafts consume that metadata instead of owning a second frontend rules table.
  ...
  - [x] Seed reusable optional item Fact definitions needed by the examples, including attribute, mana efficiency, flat effect bonus, and mana-regeneration modifier, alongside the backend-owned required weapon Fact definitions.
    - Added backend-owned optional item Fact definitions for Attribute, Mana Efficiency, Flat Effect Bonus, and Mana Regeneration Modifier.
    - Added schema-v12 checkpoint migration while preserving unrelated custom Fact definitions. These canonical Facts remain opt-in per item, editable after attachment, and informational unless an authored action, formula, or augmentation explicitly consumes them.
    - No automatic attribute attack detection, same-source-item selector behavior, timed mana regeneration, targeting, or passive-effect lifecycle was added.
  - [x] Add proficiency category metadata and seed the supplied weapon-family definitions: Long Swords, Short Swords, Spears, Shields, Pugilists, Staffs, Bows, Throwing, Knives, and Axes. Specific weapon names remain item records that reference the applicable proficiency rather than separate hardcoded resolvers.
    - Added explicit proficiency categories (`custom` and `weapon_family`) across backend state, typed requests, snapshots, generated frontend protocol, frontend domain models, and GM Proficiency Authoring.
    - Fresh state and schema-v13 migration seed the ten supplied weapon-family proficiencies while preserving existing GM-edited names/descriptions and custom records. Seeded proficiencies remain normal GM-editable records; no backend-owned lock, resolver behavior, combat automation, or weapon-specific execution rule was added.
    - Verified with focused backend proficiency/state-store/protocol tests, focused frontend proficiency/request/reducer tests, frontend lint, protocol generation, and the production TypeScript/Vite build.
  - [x] Add same-source-item evaluation context/selector support for modifiers such as `+50 to effects applied to this sword`; category-wide effects such as Fire Shard's `+10` fire damage continue to use ordinary semantic tags.
    - Added a `same_source_item` selector flag for evaluation-time numeric and roll-mode modifiers. Runtime formula contexts now carry the resolved source item relationship, and equipped item template matching compares it to the candidate bridge so same-definition inventory entries remain distinct.
    - The selector is backend-authoritative, defaults off for legacy data, does not create a new passive lifecycle, and never matches standalone/condition effects without an item source relationship. Existing tag/direct-ID selectors remain the category-wide path.
    - Frontend augmentation authoring preserves, emits, and summarizes the selector flag; no frontend gameplay calculation was added.
  - [x] Represent Parry advantage and Mana Manipulation bonuses with the existing roll-mode/numeric augmentation effects, selectors, and condition or standalone-effect application paths. Action Facts provide their authored values, but owning an action does not create a second automatic passive-effect lifecycle.
    - Added acceptance fixtures that activate Parry advantage and configured Mana Manipulation numeric/Overload effects only through explicit standalone-effect action steps. Assignment alone leaves the application collection empty.
  - [x] Build backend/frontend acceptance fixtures that author, persist, reload, display, equip, and execute the six supplied equipment examples and the Parry, Flames of Life, and Mana Manipulation actions using Facts, existing actions, and existing augmentation/condition/standalone-effect lifecycles. Keep these as fixtures/sample data rather than forcing campaign-specific records into every live state.
    - Backend fixtures submit normal typed websocket requests, equip all six examples, persist and reload an isolated checkpoint, verify direct projections do not double-apply, and execute semantic-tag and same-source-item modifiers.
    - Frontend fixtures round-trip all supplied items/actions through the existing authoring value mappers and render their authoritative equipment, Fact, action, and active-effect views.
  - [x] Verify the fixed Flames of Life example can reject insufficient mana and exchange `100` mana for `10` health through existing ordered bounded-mutation steps. Limb/injury restoration remains RP-only.
    - Insufficient mana rolls back the complete action without a patch. A valid execution emits one state version/patch containing both the 100-mana spend and bounded 10-health restoration.
  - [x] Keep weapon sharpness/dulling out of scope. Treat descriptive mana conductivity and unspecified proficiency improvements as displayed Facts/notes until concrete executable rules are supplied.
  - [x] Do not add automatic rank progression, proficiency-threshold feature hiding, timed regeneration, range enforcement, target-count enforcement, positioning, or multi-sheet targeting in this track.
  - Verified the completed examples track with 433 backend tests, 329 frontend tests, frontend lint, protocol generation, and the production TypeScript/Vite build.
- [x] Simplify GM navigation, contextual authoring, and request feedback.
  ...
  - [x] Add contextual `Create new...` dialogs to Template Builder assignment controls for Facts, Actions, Items, and Proficiencies so a GM can create a missing record without abandoning the current template draft.
    - Each assignment section now opens a compact Fact/Proficiency or large Item/Action dialog beside the outer Template Builder form. The accessible modal traps focus, supports Escape/backdrop dismissal, and blocks closure while a create is pending.
  - [x] Reuse the existing authoring value mappers, validation, typed request helpers, and focused form components in contextual dialogs rather than creating parallel payload or validation implementations.
    - Extracted the existing Fact draft controls into a shared `FactEditorForm`; contextual Proficiency, Item, and Action creation composes their established editors, Fact/effect panels, presets, validation, and submission builders.
  - [x] After an authoritative create patch arrives, select or attach the newly created record to the current template draft while preserving all other unsaved template edits. Dialog state and pending feedback remain frontend-local; catalog records remain backend-authoritative.
    - Contextual requests carry explicit entity/request IDs. A matching success plus authoritative catalog arrival attaches the record through a functional draft update; errors unlock the unchanged dialog draft, and duplicate attachment is ignored.
  - [x] Retain dedicated authoring pages for global browse/edit/delete workflows. Complex Item or Action creation may use a large dialog or offer `Continue in full editor`; avoid nested dialog chains and do not force full catalog management into Template Builder.
    - Dedicated pages remain unchanged for catalog management. Item dialogs intentionally omit nested Action Authoring navigation while still selecting existing authored actions.
  - [x] Add frontend coverage for notification suppression, relocated toolbar controls, draft preservation, authoritative create reconciliation, and automatic selection/attachment of the new record.
    - Added focused resolution, attachment-default, duplicate-prevention, untouched-draft identity, contextual-control, modal-state, and all-four-editor rendering coverage. Verified with 336 frontend tests, frontend lint, and the production TypeScript/Vite build.
