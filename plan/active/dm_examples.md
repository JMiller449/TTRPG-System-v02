# Some possible other attributes:
level
movement

# Implemented starter campaign coverage:

The validated starter session data lives in
`backend/tests/dm_examples_fixtures.py` and is exercised by
`backend/tests/test_dm_examples_acceptance.py`.

Current starter records include:

- Player templates: Ember Ranker Arin Vale, Shadowblade Mina Seo.
- Enemy templates: Red Gate Goblin, Ash Wraith.
- Campaign Attributes: Campaign Role, Gate Affinity, Guild Rank.
- Custom proficiencies: Pyromancy, Shadow Steps, Gate Lore, plus seeded weapon
  families such as Long Swords and Knives.
- Items: equippable gear and weapons, consumables, and inventory-only records.
- Conditions/effects: Shadow Bound, Bleeding, Parry Advantage, Mana Manipulation
  bonuses.
- Actions: checks, spell to-hit, spell damage, weapon actions, resource costs,
  healing, condition application/removal, consumable use, and proficiency gain.
- Encounter presets: Red Gate Scouts, Ash Wraith Threshold.

# Some presets:
Attack = (1 + Proficiency) * (d100/100) * weapon_stat_type			
Parry Attempt = Weapon Proficiency Modifier * (1d100/100) * Dexterity					
Dodge Attempt = Dexterity * (1d100/100)					
Block Attempt = Strength * (1d100/100)				
Contest Contest Roll( Counter Block, Dodge, Parry) = Weapon Proficiency Modifier * (1d100/100) * weapon_stat_type
Amount of Reactions =	Registration +	Reaction time
Reaction time =	Stamina +	Intuition	/ 2	
Lifting =	Strength			
Carry weight =	Strength			
Acrobatics = 	Dexterity +	Registration	/ 2	
Stamina =	Dexterity
Health =	Constitution 			
Endurance =	Stamina +	Constitution	/ 2	
Pain Tolerance =	Endurance +	Strength	/ 2	
Sight Distance = 	Perception			
Intuition =	Perception +	registration	/ 2	
Registration =	Perception			
 Mana =	Arcane			
Control =	Arcane +	Mana	/ 2	
Sensitivity =	Intuition +	Arcane	/ 2	
Charisma =	Will			
Mental fortitude =	Will +	Charisma	/ 2	
Courage =	mental fortitude +	Charisma	/ 2	

# Some weapons that will be seen(probably some easy presets for proficiencies)

Long swords
    Longsword
    Greatsword
    Claymore
    Zweihander
    Katana
Short swords
    Short swords
    Tachi
    Wakazashi
    arming sowrd
    falchion
Spear
    Spear
    Pike
    Javelin
    Naginata
    Halberd
Sheild
    Great sheild
    Tower sheild
    Rounds sheild
    Buckler
    Kite
Puglests
    Gauntles
    Spiked 
    knuckes
    Wraps
Staff
    Quarterstaff
    Fire Staff
    Bastion
Bows
    Long bow
    Crossbow
    Short bow
    Compound
    recurve
Throwing
    knives
    stones
    javelins
    slingshots
    tomahwak
knives
    tanto
    karambit
    bayonet
    dagger
Axe
    battle axe
    tomahawk
    viking axe
    bardichi

# Some equipment that the dm showed:

Name: Light Steps
  Type: Light Armor
  Rank: C+
  Weight: 15 lbs
  Value: 10,000 CP
  Effects:

  - 10% damage resistance
  - Advantage on Stealth checks

  Name: Never Dulls
  Type: Sword
  Rank: D
  Weight: 3 lbs
  Value: 500 CP
  Effects:

  - Deals 15 damage
  - Always remains sharp and never dulls

  Name: Fire Shard
  Type: Shard of Fire
  Rank: A
  Weight: 0.1 lbs
  Value: 1,000,000 CP
  Effects:

  - Grants the Fire attribute
  - Adds 10 fire damage to all Fire-attribute attacks

  Name: Helm of Sight
  Type: Helmet
  Rank: C
  Weight: 2 lbs
  Value: 5,000 CP
  Effects:

  - Adds 2 Perception

  Name: Pyromancy Robe
  Type: Armor
  Rank: B
  Weight: 1 lb
  Value: 100,000 CP
  Effects:

  - 25% fire resistance
  - 10% magic resistance

  Name: Sword of Mana
  Type: Sword
  Rank: S
  Weight: 3 lbs
  Value: N/A
  Effects:

  - Conducts mana at 100% efficiency
  - Adds 50 to effects applied to the sword
  - Increases mana regeneration by 25%

# Skills (Actions in our sheet)
Name: Parry
  Rank: C
  Effect:

  - Allows the user to attempt to parry any attack
  - Always grants advantage on Parry attempts
    Proficiency Improvements:

  - Ability to parry spells
  - One free use
  - Additional effects as configured

  Name: Flames of Life
  Rank: S
  Effect:

  - Grants access to regenerative fire
  - Can restore HP, limbs, and other injuries
  - Example HP cost: 100 mana restores 10 HP
  - Example limb cost: 1,000 mana restores one limb
    Proficiency Improvements:

  - Reduced mana cost
  - Increased healing
  - Increased range
  - More affected targets

  Name: Mana Manipulation
  Rank: A
  Effect:

  - Increased mana regeneration
  - Increased maximum mana
  - Advantage on Overload attempts
    Proficiency Improvements:

  - Bonuses to most related effects and checks
