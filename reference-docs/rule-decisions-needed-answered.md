...
- Tuning defines individual level costs. For current level `L`, the effective exponent is `Growth Exponent × Growth Multiplier / Milestone ^ floor(L / Milestone Interval)`. The unrounded cost of `L→L+1` is `Base XP × L^effective_exponent`, multiplied by Milestone Multiplier **only if `L+1` is a milestone**. Apply the sheet XP Growth Rate and round each cost before summing costs from level 1 through the current level to derive the lifetime target.
- Corrected defaults are **100 / 1.35 / 25 / 1.08 / 1.02 / 10** (Base XP / Growth Exponent / Milestone Interval / Milestone Multiplier / Growth Multiplier per Milestone / Rounding Increment). Level 24→25 gets the 1.08 bump using exponent 1.35; level 25→26 uses exponent 1.377 without the bump. At level 50, the exponent is 1.40454. An interval of 100 means milestones 100, 200, 300, and so on. A growth multiplier of 1 disables exponent escalation. Both milestone multipliers use 1 as neutral, and 1.08 means an 8% increase. This supersedes the spreadsheet’s earlier additive exponent increase. At normal growth the first three costs are 100, 250, 440, giving lifetime targets 100, 350, 790.
