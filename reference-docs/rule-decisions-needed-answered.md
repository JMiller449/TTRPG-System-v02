...
### Derived XP Goals (2026-09-06)

Q: How should the app derive the XP threshold in Chip TTRPG System §8 while leaving advancement under DM control?

A: The DM selects one campaign-wide progression mode: tuning controls or a deterministic math equation over numeric sheet Attributes. The result is a lifetime-XP target for advancing from the character's current manually assigned Level; XP is never spent or reset by advancement. Remaining XP is max(0, target − lifetime earned XP). Reaching the target only marks readiness; the DM changes Level explicitly.

- Tuning defines individual level costs. For current level `L`, the effective exponent is `Growth Exponent + floor(L / Milestone Interval) × Growth Increase / Milestone`. The unrounded cost of `L→L+1` is `Base XP × L^effective_exponent`, multiplied by Milestone Multiplier **only if `L+1` is a milestone**. Apply the sheet XP Growth Rate and round each cost before summing costs from level 1 through the current level to derive the lifetime target.
- Corrected defaults are **100 / 1.35 / 25 / 1.08 / 0.02 / 10** (Base XP / Growth Exponent / Milestone Interval / Milestone Multiplier / Growth Increase per Milestone / Rounding Increment). Level 24→25 gets the 1.08 bump using exponent 1.35; level 25→26 uses exponent 1.37 without the bump. At level 50, the exponent is 1.39. An interval of 100 means milestones 100, 200, 300, and so on. Zero growth increase disables exponent escalation. At normal growth the first three costs are 100, 250, 440, giving lifetime targets 100, 350, 790.
- Equation mode defines that same lifetime target using current instance Attributes, such as `100 * @level ** 2`. `@{attribute_id}` supports IDs containing punctuation. Equations support the shared runtime's arithmetic and min/max/floor/ceil/round functions, without dice. Exponents are literal constants from 0 through 16; nested powers are disallowed.
- Every template and instance has a required positive numeric XP Growth Rate Attribute, default 1. Both modes multiply their result by this value. Below 1 is easier and above 1 harder. Templates seed new instances; existing instances retain their own copies.
- Tuning rounds each individual cost down after growth to the configured positive whole-number multiple, with a minimum of one multiple. Equation mode rounds down to whole XP, minimum 1. This follows §2.2's general round-down rule.
